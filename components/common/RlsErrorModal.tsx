    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="rounded-lg p-6 max-w-md w-full mx-4 border shadow-2xl" style={{ background: T.inkSoft, borderColor: T.inkLine }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 mb-4">
          <AlertCircle size={20} style={{ color: T.rust }} />
          <h3 className="text-lg font-semibold" style={{ color: T.text, fontFamily: "'Fraunces', serif" }}>Erreur d'accès</h3>
        </div>
        <p className="text-sm mb-4" style={{ color: T.text }}>{error.message}</p>
        <details className="mb-4">
          <summary className="text-xs cursor-pointer hover:underline" style={{ color: T.gold }}>Voir détails techniques</summary>
        ...
        <a href={error.docLink || '/docs/rls-policy'} target="_blank" rel="noopener noreferrer" className="text-xs underline block mb-4" style={{ color: T.gold }}>
